...
def topLevelServer():
	...
	#first, the top level server has to load information from the PROJI-DNSTS.txt
	...
	f_input = open("PROJI-DNSTS.txt", "r")
	...
	#creating a socket on top level server side
	try:
	...
	server_binding = ('', port)
	tSocket.bind(server_binding)

	#enable this socket to start listening to connections
	tSocket.listen(1)

	#accepting a connection from client at ip 'addr'. 
	#'csockid' is the socket id for the connecting socket (on server side) for this particular connection 
	csockid, addr = tSocket.accept()
	...
	while True:
		...
		if not found:
			output_data = decoded_data + ' - Error:HOST NOT FOUND'
			csockid.send(output_data.encode('utf-8'))
...
if __name__ == "__main__":
    ...
    #getting the command line input: port through which top level server listens for requests
    port = int(sys.argv[1])

    #Initiate a thread that will run the topLevelServer() function
    t1 = threading.Thread(name='topLevelServer', target=topLevelServer)
    t1.start()
